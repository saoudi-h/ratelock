...
export function UnifiedTimelineBase({
    ...
    const [simulationStartTime] = useState(() => startTime ?? Date.now())
    ...
    const positionedEvents = useMemo(() => {
        return events.map(event => {
            ...
            const accent =
                accentByEvent?.(event) ?? (event.allowed ? DEFAULT_ALLOWED : DEFAULT_DENIED)
        ...
        <div
            ...
            {staticWindows.map(window => {
                ...
                return (
                    <div
                        key={window.id}
                        className={`
                          absolute inset-y-4 z-0 rounded-lg border-x
                          border-dashed
                          ${tone}
                        `}
                        style={{
            ...
            <div
                ...
                {positionedWindows.map(window => {
                    ...
                    return (
                        <div
                            key={window.id}
                            className={`
                              absolute inset-y-4 rounded-lg transition-all
                              duration-300
                              ${tone}
                            `}
                            style={{
                ...
                {positionedEvents.map(({ event, left, accent }) => {
                    const isNew = now - event.timestamp < 600
