...
For a global interception point, Next.js also supports `middleware.ts`, which runs before routing on every matched path. It executes in the Edge runtime, but the Redis client used in this recipe is connection-oriented and is not supported there. Use a Node.js route handler with this setup; an HTTP-native backend adapter would be a separate integration.
