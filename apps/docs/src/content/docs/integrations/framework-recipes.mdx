...
Web-standard frameworks (Next.js, SvelteKit, Astro, Remix, SolidStart) share one shape: handlers receive a standard `Request` and return a standard `Response`. There is no middleware pipeline to hook into, so instead of a dedicated package you wire a limiter directly inside each handler.
...
For a global interception point, Next.js also supports `middleware.ts`, which runs before routing on every matched path. The connection-oriented client used in the recipe above is not supported in the Edge runtime. Use `@ratelock/upstash` with the official Upstash REST client instead:
...
Install it with `npm install @ratelock/upstash @upstash/redis`. Keep the REST
token in the framework's server-side secret configuration.
...
SolidStart API routes live in `src/routes` and export functions named after the HTTP methods they handle. The handler receives an `APIEvent`; its `request` property is a standard `Request`.

```ts
// src/routes/api/things.ts
