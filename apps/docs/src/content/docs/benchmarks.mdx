...
| :--- | :---: | :---: | :---: | :---: |
| **Raw Redis** | 127,719 | 1,000 | 0.16% | reference |
| **+ withRetry** | 112,688 | 1,000 | 0.16% | -12% (slower, same result) |
| **+ withCircuitBreaker** | 85,630 | 143 | 0.03% | -33% (trips after 3 errors) |
| **+ withFallback** | 134,482 | 80,896 | 10.13% | **+5% throughput, errors hidden** |

Two non-obvious results here:

- `withRetry` is **slower** than raw, not faster. The reason: under extreme spam the limiter denies 99.9% of calls within the first few milliseconds. Re-running a denied call does not change the answer — it just adds latency. Retry only helps when the answer would have been different, and a "denied" decision from the rate limiter is final.
- `withCircuitBreaker` is the **worst** outcome: 143 allowed (vs 1,000 for raw). The breaker opens after 3 errors and starts fast-failing, which means even requests that the limiter would have allowed are now blocked. The breaker is designed to protect the *backend*, not the *user* — and 10% transient errors is exactly the regime where it is too aggressive for a limiter that already denies most traffic.
...
This is a known gap. A future addition — `withTimeout(p, { budgetMs: 10 })` — would let a decorator abort a slow call and treat it as a failure, at which point the existing retry / circuit-breaker / fallback stack would react correctly. The current benchmarks do not include this decorator.
...
| Limiter | Throughput (ops/sec) | Allowed count | Allowed % | vs raw |
| :--- | :---: | :---: | :---: | :---: |
| **Raw Redis** | 128,882 | 0 | 0.00% | reference |
| **+ withRetry** | 17,545 | 0 | 0.00% | -86% (retries 3x, still fails) |
| **+ withCircuitBreaker** | 80,550 | 0 | 0.00% | -37% (trips fast, no recovery) |
| **+ withFallback** | **120,876** | **719,904** | **100.00%** | **flat, errors hidden** |

The headline. When Redis is hard-down, raw, withRetry, and withCircuitBreaker all serve **0** requests. `withFallback` serves **719,904** of them at near-baseline throughput (120,876 ops/sec vs 136,337 for raw-on-healthy, a 11% gap that is the cost of the `try`/`catch` wrapping). The fallback policy is "allow" with a local synthetic result, so the service keeps accepting traffic and the user-facing system stays up.

The Retry decorator is the worst choice in this scenario: 86% slower than raw, because it tries the failing call 3 times in series (`baseDelayMs: 2, maxDelayMs: 10`) before giving up. The CircuitBreaker is faster than retry but still serves nothing, because opening the circuit means every subsequent call returns the configured "deny" without consulting the backend — there is no fallback configured for the breaker.
...
`withCircuitBreaker` and `withRetry` are not useless — they shine in scenarios the current bench does not model (circuit-breaker protects the backend from a tight retry loop; retry smooths out one-off blips on a primary before the fallback activates). But on their own, with a single-tier Redis backend, they cost more than they save. The right place for them is in front of the primary when the primary feeds a `withFallback`.
