...
| `prefix` | `string` | Key prefix (default: `"tb"`) |
...
| `remaining` | `number` | Tokens remaining after this check |
| `tokens` | `number` | Same as `remaining` |
| `refillTime` | `number` | Milliseconds until next token (0 if allowed) |
