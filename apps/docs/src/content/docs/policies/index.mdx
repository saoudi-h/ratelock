...
> **Note:** Most users should use the [built-in resilience options](/docs/engines) at limiter creation time. Standalone wrappers are for advanced scenarios where you need flexibility beyond the defaults.
...
const testLimiter = withFallback(limiter, { fallback: 'allow' })
```
