...
Each table also gets an index on `expires_at` to speed up cleanup of expired rows.
...
The PostgreSQL engine runs periodic cleanup of expired rows every 5 minutes. You can also trigger it manually by passing a `PgDriver` instance:
...
const deletedCount2 = await cleanupExpired(postgresDriver(sql))
```

Wrap your existing client with `pgDriver(pool)` or `postgresDriver(sql)` to obtain a `PgDriver`. Both are exported from `@ratelock/postgres`.
