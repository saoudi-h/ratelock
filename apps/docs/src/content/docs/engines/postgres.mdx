...
All limiters in the PostgreSQL engine accept these shared base options (supporting both `connectionString` and `url` interchangeably):
...
const limiter = await fixedWindow({ connectionString: 'postgres://...', limit: 100, windowMs: 60_000 })
