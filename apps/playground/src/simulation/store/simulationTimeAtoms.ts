 ...
 */
export const updateRealTimeNowAtom = atom(null, (_, set) => {
 ...
 */
export const simulationTimeAtomFamily = atomFamily(() => atom<number>(Date.now()))
 ...
 */
export const lastKnownTimeAtomFamily = atomFamily(() => atom<number>(Date.now()))

/**
 * Simulation running state for each strategy
 * @returns Atom containing boolean simulation state
 */
export const isSimulationRunningAtomFamily = atomFamily(() => atom<boolean>(false))
