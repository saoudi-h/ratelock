...
import {
    resetSimulationAtomFamily,
    startSimulationAtomFamily,
    stopSimulationAtomFamily,
} from '../store/simulationControlAtoms'
...
export function useRateLimitStrategy(strategyId: RateLimitStrategy): StrategyHookReturn {
    ...
    const [hasInitialStarted, setHasInitialStarted] = useAtom(
        hasInitialStartedAtomFamily(strategyId)
    )
