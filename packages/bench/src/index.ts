...
const STRATEGIES: Strategy[] = ['fixed-window', 'sliding-window', 'token-bucket', 'individual-fixed-window']
...
async function main() {
  ...
  console.log(`  Done. ${suite.results.filter((r) => !r.error).length}/${suite.results.length} scenarios completed.`)
  console.log()
}
