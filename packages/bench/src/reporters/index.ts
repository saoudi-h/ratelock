...
export function reportConsole(result: BenchmarkResult): void {
  ...
  console.log(`  Requests:    ${result.totalRequests.toLocaleString()} total, ${result.successes.toLocaleString()} ok, ${result.failures.toLocaleString()} fail`)
  console.log(`  Latency:     p50=${fmt(l.p50)} p95=${fmt(l.p95)} p99=${fmt(l.p99)} avg=${fmt(l.avg)}`)
  console.log(`  Memory:      ${result.memory.heapUsedMb} MB delta, ${result.memory.heapTotalMb} MB total`)
