...
async function main() {
  ...
  try {
    ...
    await c1.quit()
  } catch (e: any) { console.log(`  redis (node-redis): SKIPPED (${e.message})\n`) }
  ...
  try {
    ...
    c2.disconnect()
  } catch (e: any) { console.log(`  redis (ioredis): SKIPPED (${e.message})\n`) }
  ...
  try {
    ...
    await pool.end()
  } catch (e: any) { console.log(`  postgresql (pg): SKIPPED (${e.message})\n`) }
}
