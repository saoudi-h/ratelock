...
async function main() {
    ...
    await v1.connect()
    const l1 = await fixedWindow({ client: v1, limit: 100000, windowMs: 60000 })
    await bench('Valkey    (node-redis client)', id => l1.check(id))
    await v1.quit()

    // Valkey via ioredis client
    const v2 = new IORedis('redis://localhost:6381')
    const l2 = await fixedWindow({ client: v2, limit: 100000, windowMs: 60000 })
    ...
    await r1.connect()
    const l3 = await fixedWindow({ client: r1, limit: 100000, windowMs: 60000 })
    ...
    const l4 = await fixedWindow({ client: r2, limit: 100000, windowMs: 60000 })
