...
const mockStrategyFactory = vi.fn(
    (storage: Storage, options: any) => new MockStrategy(storage, options)
)
const mockStorageFactory: StorageFactory<MockStorage, any> = vi.fn(
    async (_config: any) => new MockStorage()
)
