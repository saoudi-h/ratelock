...
- Root `pnpm test` starts Vitest in **watch mode**; use `vitest run` or `pnpm test:coverage` for one-shot. Turbo `test` has no `dependsOn: build` — build first before testing.
